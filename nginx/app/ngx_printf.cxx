...
u_char *ngx_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
    ...
    while (*fmt && buf < last) //每次处理 fmt中的 一个字符
    {
        if (*fmt == '%') // %开头的一般都是需要被可变参数
        {
            //-----------------变量初始化工作开始-----------------
            //++fmt是先加后用，也就是fmt先往后走一个字节位置，然后再判断该位置的内容
            zero = (u_char)((*++fmt == '0') ? '0' : ' '); // 判断用0填充还是用空格填充

            width = 0;      //格式化宽度，只对数字格式有效，比如%d,%f
            sign = 1;       //1表示有符号数，0表示无符号数
            hex = 0;        //是否以16进制形式显示(比如显示一些地址)，0：不是，1：是，并以小写字母显示a-f，2：是，并以大写字母显示A-F
            frac_width = 0; //小数点后位数字，一般需要和%.10f配合使用，这里10就是frac_width；
            i64 = 0;        //%d对应的可变参中的实际数字，会保存在这里
            ui64 = 0;       //%ud对应的可变参中的实际数字，会保存在这里
...
{
    //temp[21]
    u_char *p, temp[NGX_INT64_LEN + 1]; // NGX_INT64_LEN = 20
    size_t len;
    uint32_t ui32;
